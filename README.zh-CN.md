...
5. 内部生成统一风格的 image-2 图片提示词，不再要求用户确认提示词，直接进入图片生成。
6. 批量生成 16:9 PPT 图片，并请用户确认最终图片。
7. 图片确认后，自动组装成可播放的 16:9 HTML 演示文件。
8. 结合大纲和最终图片，按演讲时长逐页生成可直接照读的口语化讲稿。

后续如需修改单页，技能会重新梳理并确认该页的逻辑、内容、例子和视觉重点，再生成替换图片；整套已确认风格会继续沿用，提示词无需再次确认。
